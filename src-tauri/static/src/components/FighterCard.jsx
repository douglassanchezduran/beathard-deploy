...
const FighterCard = ({ 
  ...
  return (
    <div className={getAnimationClasses()}>
      <div className={`bg-gradient-to-br ${colorScheme.gradient} backdrop-blur-sm rounded-3xl border-4 ${colorScheme.border} p-8 transform hover:scale-105 transition-all duration-300 hover:shadow-2xl ${colorScheme.shadow}`}>
        {/* Foto del peleador */}
        ...
        {/* Nombre del peleador */}
        <h2 className="text-5xl font-black text-white mb-4 text-center tracking-wider">
        ...
        <div className="space-y-4">
          <div className="flex justify-between items-center">
            <span className="text-gray-300 text-lg">Peso:</span>
            <span className="text-white font-bold text-xl">
              {competitor?.weight ? `${competitor.weight} kg` : "-- kg"} 
              <span className="text-gray-400 text-base ml-1">
                {competitor?.weight ? `(${kgToLbs(competitor.weight)} lbs)` : "-- lbs"}
              </span>
            </span>
          </div>
          
          <div className="flex justify-between items-center">
            <span className="text-gray-300 text-lg">Altura:</span>
            <span className="text-white font-bold text-xl">
              {competitor?.height ? `${competitor.height} cm` : "-- cm"} 
              <span className="text-gray-400 text-base ml-1">
                {competitor?.height ? `(${cmToFeetInches(competitor.height)})` : "--"}
              </span>
            </span>
          </div>
